/**
 * api/gemini.js — Proxy serverless do Assistente CBKS (Vercel Functions, Node.js runtime).
 *
 * Publicado na MESMA origem do site (mesmo projeto/domínio Vercel), portanto
 * não depende de CORS entre domínios diferentes — o navegador chama /api/gemini
 * como um caminho relativo do próprio site.
 *
 * A chave da API do Gemini é lida de process.env.GEMINI_API_KEY (variável de
 * ambiente configurada no painel do projeto na Vercel — NUNCA hardcoded aqui).
 */
 ...
 * Arquivos dentro de /api viram rotas automaticamente: este arquivo
 * corresponde a POST /api/gemini.
 */
export default async function handler(req, res) {
  // Mesma origem — CORS não é necessário para o uso normal do site, mas os
  // headers abaixo permitem também testar a função isoladamente (ex: curl,
  // Postman) sem bloqueio de navegador durante o desenvolvimento.
